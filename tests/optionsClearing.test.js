describe('Option clearing adjustments', () => {
  ...
  test('assigned underlying path avoids floating-point drift', async () => {
    jest.resetModules();
    ...
    const Clearing = require('../src/clearing.js');
    const pos = { contracts: 0.1, avgPrice: 100, margin: 0.1, lastMark: null };

    Clearing.applyAssignedUnderlyingPosition(pos, 0.2, 110, 0.2);

    expect(pos.contracts).toBe(0.3);
    expect(pos.margin).toBe(0.3);
    expect(pos.avgPrice).toBeCloseTo(106.6666666667, 10);
  });
});
