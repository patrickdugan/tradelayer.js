...
The registered verifier rejection receipt is `artifacts/zk_signed_channel_transfer/tx34_verifier_rejection_demo_latest.json`. It decodes a compact `z2` tx34 anchor, resolves the candidate DA blob, runs `Validity.validateZkBatchMovement`, and only then calls `Logic.zkBatchMove`. It shows the current envelope accepted and applied by `rust-wasm`, then rejects three tampered variants before logic application: a self-consistent DA blob whose `proofSummary.proofSha256` no longer equals the envelope `proofHash`, a raw DA mutation without recomputing the envelope, and an unregistered verifier WASM hash.
The proof-artifact receipt is `artifacts/zk_signed_channel_transfer/tx34_full_proof_artifact_demo_latest.json`. It sets `TL_ZK_REQUIRE_PROOF_ARTIFACT=1`, materializes the full `proof.json`, hashes its bytes inside the tx34 validation path, requires that hash to equal `proofHash`, and applies the channel movement only after that check. This is proof-byte availability and binding, not yet cryptographic STWO proof verification over the proof bytes.
