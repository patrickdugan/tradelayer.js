...
The registered verifier rejection receipt is `artifacts/zk_signed_channel_transfer/tx34_verifier_rejection_demo_latest.json`. It shows the current envelope accepted by `rust-wasm`, then rejects three tampered variants: a self-consistent DA blob whose `proofSummary.proofSha256` no longer equals the envelope `proofHash`, a raw DA mutation without recomputing the envelope, and an unregistered verifier WASM hash.
