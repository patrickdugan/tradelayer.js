...
const Logic = {
	...
	async activateTradeLayer(txType, block) {
		...
		const activationResult = await tlActivation.activate(txType, block);
	...
	async tokenIssue(sender, initialAmount, ticker, url = '', whitelistId = 0, isManaged = false, backupAddress = '', isNFT = false) {
		...
		try {
			var newPropertyId = await propertyList.createToken(ticker, initialAmount, tokenType, whitelistId, backupAddress);
	...
	async sendAll(senderAddress, receiverAddress) {
		...
		const senderBalances = tallyMap.getAddressBalances(senderAddress);
	...
	async onChainTokenToToken(fromAddress, offeredPropertyId, desiredPropertyId, amountOffered, amountExpected, txid, blockHeight) {
		...
		const orderbook = await orderBook.load(pairKey);
		...
		if (txInfo?.blockhash) {
			confirmedBlock = await TxUtils.getBlockHeight(txInfo.blockhash)
		}
	...
	async tradeContractOnchain(contractId, price, amount, side, insurance, contractsRegistry, blockTime, txid, sender) {
		...
		const orderbook = await orderBook.load(contractId);
	...
	async getMarketPrice(contract) {
		let marketPrice;
		const contractInfo = txRegistry.getContractInfo(contract.id)
		if (contractInfo?.type === 'oracle') {
		...
		} else if (contractInfo?.type === 'native') {
