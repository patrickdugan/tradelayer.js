...
const Validity = {
    ...
    validateActivateTradeLayer: async (txId, params, sender) => {
        ...
        // const isAlreadyActivated = await tlActivation.isTxTypeActive(params?.txTypeToActivate);
        // //console.log('isAlreadyActivated '+isAlreadyActivated, params.txTypeToActivate)
        // const activationBlock = await tlActivation.checkActivationBlock(params?.txTypeToActivate)
    ...
    validateTokenIssue: async (params) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(1);
    ...
    validateSend: async (sender, params, txId) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(2);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateTradeTokenForUTXO: async (params) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(3);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateCommitToken: async (params, block, txid) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(4);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateOnChainTokenForToken: async (sender, params, txId) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(5);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateCancelOrder: async (sender, params, txid) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(6);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateCreateWhitelist: async (sender, params, txid) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(7);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateUpdateAdmin: async (sender, params, txid) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(8);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateIssueAttestation: async (sender, params, txid) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(9);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateRevokeAttestation: async (sender, params, txid) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(10);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateGrantManagedToken: async (sender, params, txid) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(11);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateRedeemManagedToken: async (sender, params, txid) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(12);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateCreateOracle: async (sender, params, txid) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(13);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validatePublishOracleData: async (sender, params, txid) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(14);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        } else {
    ...
    validateCloseOracle: async (sender, params, txid) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(15);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateCreateContractSeries: async (sender, params, txid) => {
        ...
        if (params.native === false) {
            const validOracle = await oracleList.getOracle(params.underlyingOracleId) !== null;
            if (!validOracle) {
                params.valid = false;
        ...
        if (params.native === true && params.onChainData) {
            ...
            for (const pid of params.onChainData) {
                if (pid !== null && !(await propertyList.getProperty(pid))) {
                    validNatives = false;
                    break;
                }
            }
        ...
        if (params.notionalPropertyId !== null && params.native == true) {
            const validNotionalProperty = await propertyList.getProperty(params.notionalPropertyId) !== null;
        ...
        const validCollateralProperty = await propertyList.getProperty(params.collateralPropertyId) !== null;
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(16);
    ...
    validateExerciseDerivative: async (params, derivativeRegistry, marginMap) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(17);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateTradeContractOnchain: async (params, sender, block) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(18);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateTradeContractChannel: async (params, channelRegistry, whitelistRegistry, contractRegistry) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(19);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateTradeTokensChannel: async (params, channelRegistry, whitelistRegistry) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(20);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateWithdrawal: async (params, sender, block) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(21);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateTransfer: async (params, channelRegistry, tallyMap) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(22);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
    ...
    validateSettleChannelPNL: async (params, channelRegistry, marginMap) => {
        ...
        const isAlreadyActivated = await tlActivation.isTxTypeActive(23);
        if (isAlreadyActivated == false) {
            params.valid = false
            params.reason += 'Tx type not yet activated '
        }
